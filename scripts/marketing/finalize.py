...
text(d,(64,530),'my-boats.vercel.app',24,bold=True)
...
captions([(s['duration'],s['title'],s['sub']+(' Take the helm at my-boats.vercel.app.' if s.get('end') else '')) for s in SCENES],OUT/'feature-en.vtt')
captions([(duration,title,subtitle+(' Take the helm at my-boats.vercel.app.' if i==9 else '')) for i,duration,title,subtitle in CUTS],OUT/'short-en.vtt')
